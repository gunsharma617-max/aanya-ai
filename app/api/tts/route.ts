...
- use natural pauses and a comfortable speaking pace
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!audioBase64) {
      ...
      return NextResponse.json(
        { error: "No audio was returned by Gemini TTS." },
        { status: 502 }
      );
    }
    ...
    return NextResponse.json(
      { error: "Unable to generate Aanya's voice." },
      { status: 500 }
    );
  }
}
